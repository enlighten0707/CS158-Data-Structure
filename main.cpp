...
bool isEqual(std::deque<Int> &a, sjtu::deque<Int> &b) {
    ...
    //sjtu::deque<Int>::iterator itr=b.begin();
    //for(;itr!=b.end();++itr) std::cout<<(*itr).data<<std::endl;
    ...
    if (resultA.size() != resultB.size()) {
        //std::cout<<"size!\n"<<resultA.size()<<' '<<resultB.size()<<std::endl;
        return false;
    }
    for (int i = 0; i < (int)resultA.size(); i++) {
        if (resultA[i] != resultB[i]) {
            //std::cout<<i<<" : "<<resultA[i].data<<' '<<resultB[i].data<<std::endl;
            return false;
        }
    }
    return true;
...
std::pair<bool, double> pushBackChecker() {
    ...
    for (int i = 0; i < N; i++) {
        ...
        a.push_back(tmp);
        b.push_back(tmp);
        //std::cout<<tmp<<' '<<a[a.size()-1].data<<' '<<b[b.size()-1].data<<std::endl;
    }
    timer.stop();
    //b.print();
    if (!isEqual(a, b)) {
